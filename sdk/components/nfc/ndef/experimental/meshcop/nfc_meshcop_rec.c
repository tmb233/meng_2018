...
{
    static meshcop_payload_desc_t meshcop_payload_desc;
    static const uint8_t          static_meshcop_type[] = MESHCOP_NDEF_TYPE;

    assert(p_meshcop_data != NULL);

    meshcop_payload_desc.p_meshcop_data   = (uint8_t const *)p_meshcop_data;
    meshcop_payload_desc.meshcop_data_len = sizeof(nfc_meshcop_data_t);

    NFC_NDEF_GENERIC_RECORD_DESC_DEF(meshcop_rec,
                                     TNF_MEDIA_TYPE,
                                     NULL,
                                     0,  // no id
                                     static_meshcop_type,
                                     sizeof(static_meshcop_type) - 1,
                                     nfc_meshcop_payload_constructor,
                                     &meshcop_payload_desc);

   return &NFC_NDEF_GENERIC_RECORD_DESC(meshcop_rec);
}
